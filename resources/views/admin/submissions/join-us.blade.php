...
@section('title', 'Join Us Submissions')
    ...
    <table class="w-full text-sm">
        ...
        <tbody>
            ...
            <tr class="border-b border-gray-50 hover:bg-gray-50 transition">
                <td class="py-3 px-4 font-medium text-gray-800">{{ $sub->name }}</td>
                ...
                <td class="py-3 px-4 text-right space-x-2">
                    ...
                    <form method="POST" action="{{ route('admin.submissions.join-us.destroy', $sub->id) }}" class="inline" onsubmit="return confirm('Delete this submission?')">
                        @csrf @method('DELETE')
...
<div id="detailModal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 relative">
        ...
        <h3 class="font-bold text-gray-800 text-lg mb-4">Join Us Submission</h3>
...
function showDetail(id) {
    ...
    document.getElementById('detailContent').innerHTML = `
        <div class="flex gap-2"><span class="font-semibold text-gray-600 w-20 shrink-0">Name:</span><span class="text-gray-800">${sub.name}</span></div>
